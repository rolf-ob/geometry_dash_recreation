...
def draw_background(game, screen):
    for obj, points in zip(game.background, game.background_points): #! Cache and only draw if within camera view
        draw_polygon(game, screen, points, obj.color, 0)
        draw_polygon(game, screen, points, obj.outline, 1)

def draw_objects(game, screen):
    for obj, points in zip(game.objects, game.object_points): #! Cache and only draw if within camera view
...
def draw_decoration(game, screen):
    for obj, points in zip(game.decoration, game.decoration_points): #! Cache and only draw if within camera view
        draw_polygon(game, screen, points, obj.color, 0)
        draw_polygon(game, screen, points, obj.outline, 1)

def draw_hitbox_trail(game, screen):
    for box, points in zip(game.hitbox_trail[:-1], game.hitbox_trail_points[:-1]): #! Cache and only draw if within camera view
        draw_polygon(game, screen, points, box.outline, 1)
    draw_polygon(game, screen, game.hitbox_trail_points[-1], game.player.color, 1)

def draw_heading_trail(game, screen): #! Add heading trail
    for box, points in zip(game.heading_trail, game.heading_trail_points):
        draw_polygon(game, screen, points, box.outline, 1)

def draw_wave_trail(game, screen): #! Cache and only draw if within camera view
...
def draw_debug(game, screen):
    ...
    screen.blit(game.text_cache.get_surface(f"Level: {game.current_level % game.level_amount}", (100,)*3), (10, 30))
    screen.blit(game.text_cache.get_surface(f"Player height: {game.player.y}", (100,)*3), (10, 50))
    screen.blit(game.text_cache.get_surface(f"Zoom: {round(game.camera_zoom, 1)}", (100,)*3), (10, 70))
    screen.blit(game.text_cache.get_surface(f"Noclip deaths: {game.noclip_deaths}", (100,)*3), (10, 90))
...
def draw(game):
    ...
    if game.show_heading and not (game.dead > 0 or game.completed):
        draw_heading_trail(game, game.screen)
    
    #render player and wave trail
    if not (game.show_hitboxes and (game.completed or game.dead > 0 or game.paused)):
    ...
    game.screen.blit(game.text_cache.get_surface(f"{game.level_data[0][2]}", (100,)*3), (10, game.height - 30))
